    ...
    def random_taxos_helper(sizes: list, group_sizes: list, instruments: list, taxo={}):
        ...
        # if we have more groups to build iterate through the number of groups and continue recursively
            ...
            for i in range(sizes[0]):
                ...
                total_groups[0] += 1
                result = random_taxos_helper(
                    sizes[1:], group_sizes[1:], subset, taxo={})
...
def generate_random_taxonomies(num_taxonomies: int, sizes: list, size_type='single', seed=42):
    ...
    for i in range(num_taxonomies):
        ...
        if size_type == 'single':
            ...
            # basice value error
            raise ValueError(
                f'"{size_type}" is an invalid string for size_type. Please use "single" or "multi".')

        # taxo_size = taxo_size[:(depth-1)]

        # call helper function to get a dict for the current random taxonomy
        # breakpoint()
        new_taxo_size = list(taxo_size)
        for j, t in enumerate(taxo_size):
            random.seed(i**j)
            if t >= 2:
                new_taxo_size[j] += random.choice([-1, 0, 1, ])

        curr_taxo = _generate_random_taxonomy(new_taxo_size, seed+i)
